...
def run_fahp_for_airbnb():
    # Step 1: Load dataset
    df = pd.read_excel('E:/Qurratulain/FHP-performance-testing-mobile-apps/dataset/AIRBNB_combined.xlsx')

    # Step 2: Define the criteria and pairwise comparisons
    criteria = ['Elapsed Time', 'Throughput', 'Load/Network', 'Latency']

    # Pairwise comparisons based on linguistic judgments for AIRBNB
    ...
    possibilities = {
        ...
        ('Load/Network', 'Latency'): 0.65
    }
    
    priority_weights = calculate_priority_weights(possibilities, criteria)

    # Step 5: Save the results
    save_results_to_csv_and_visualize(priority_weights, 'results/airbnb_fahp_results.csv','AirBnb')
